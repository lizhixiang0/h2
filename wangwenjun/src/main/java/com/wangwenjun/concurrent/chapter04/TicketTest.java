 ...
 *
 *  需要搞明白,变量什么时候从线程空间(cpu寄存器和高速缓存的抽象描述)写入主内存！什么时候又从主内存读入内存空间？
 ...
 *          1、线程中释放锁时
 *          2、线程切换时
 *          3、线程sleep休眠，IO操作  (此类cpu空闲时)
 *
 *   其实synchronized就是强制的串行化，每次只允许一个线程执行！并发效率相对是很低的！
 *
 *   留一个问题：单核cpu是否有可见性问题？为什么？ 应该么得
 *
 *
 *
 *
...
public class TicketTest implements Runnable {
     ...
     * 不可以,虽然通过MESI协议保证了多核下可见性，但未保证原子性！
     */
    private int index = 1;
    /**
     * 多线程共享变量
