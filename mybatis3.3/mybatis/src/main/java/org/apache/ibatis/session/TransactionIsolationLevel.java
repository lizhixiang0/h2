 ...
 * 2.幻读（针对其他事务提交前后，读取数据条数的对比，也就是说幻读针对的是insert操作）,幻读是指同样一笔查询在整个事务过程中多次执行后，查询所得的结果集是不一样的,幻读针对的是多笔记录
 * 3.不可重复读（针对其他事务提交前后，读取数据本身的对比,针对的update操作），如果事务1在事务2的更新操作之前读取一次数据，在事务2的更新操作之后再读取同一笔数据一次，两次内容是不同的
